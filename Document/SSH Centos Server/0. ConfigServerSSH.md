...
log "systemctl stop sshd"
sudo systemctl stop sshd &>> "$LOG_FILE"

log "yum remove openssh-server -y" 
sudo yum remove openssh-server -y &>> "$LOG_FILE"

log "yum install openssh openssh-server openssh-clients openssl-libs -y"
...
log "systemctl enable sshd "
sudo systemctl enable sshd &>> "$LOG_FILE"

# Cấu hình tường lửa
log "sudo firewall-cmd --add-port=22/tcp --permanent"
...
# Cài đặt Docker
log "sudo yum install -y yum-utils"
sudo yum install -y yum-utils &>> "$LOG_FILE"

log "sudo yum-config-manager --add-repo https://download.docker.com/linux/centos/docker-ce.repo"
sudo yum-config-manager --add-repo https://download.docker.com/linux/centos/docker-ce.repo &>> "$LOG_FILE"

log "sudo yum install docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin"
...
# Cài Portainer
log "docker run -d --name portainer --restart=always -p 9000:9000 -v /var/run/docker.sock:/var/run/docker.sock -v portainer_data:/data portainer/portainer-ce"
