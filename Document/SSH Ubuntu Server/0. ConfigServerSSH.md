...
sudo docker run -d \
  ...
  --privileged \
  -p 8080:80 \          # Thay đổi 8080 thành port bạn muốn sử dụng cho HTTP
  -p 8443:443 \         # Thay đổi 8443 thành port bạn muốn sử dụng cho HTTPS
...
# Mở dashboard của Minikube
minikube dashboard >> $LOG_FILE 2>&1
log_result "Mở dashboard của Minikube"

log_result "Hoàn tất cấu hình SSH server, cài đặt Docker và Minikube"
